...
Q_COREAPP_STARTUP_FUNCTION(registerSettingsTypes)
...
void SettingsUiBuilder::loadSection(const SettingsSection &section)
{
	...
	//get the input factory and generate a default settings one if not set
	auto inputFactory = QuickPresenter::inputViewFactory();
	auto settingsFactory = dynamic_cast<SettingsInputViewFactory*>(inputFactory);
	QScopedPointer<SettingsInputViewFactory> localFactoryPtr;
	if(!settingsFactory) {
		settingsFactory = new SettingsInputViewFactory();
		localFactoryPtr.reset(settingsFactory);
	}
	...
	foreach(auto group, section.groups) {
		foreach(auto entry, group.entries) {
			...
			element->delegateUrl = settingsFactory->getDelegate(entry.type, entry.properties);
